...
extern "C" bool _fastcall MyProc1();
extern "C" double RadToDegAsm(double rad);
...
bool CheckSSECpp() {
    int cpuInfo[4];
    __cpuid(cpuInfo, 1);  // We use 1 for processor info and feature bits

    bool sseAvailable = (cpuInfo[3] & (1 << 25)) != 0;  // Check SSE availability
    bool sse2Available = (cpuInfo[3] & (1 << 26)) != 0; // Check SSE2 availability

    std::cout << "SSE Available: " << (sseAvailable ? "Yes" : "No") << std::endl;
    std::cout << "SSE2 Available: " << (sse2Available ? "Yes" : "No") << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    *((int*)(CPUString + 8)) = CPUInfo[2];
    ...
    double radians = 17.0;
    double degrees = RadToDegNMEACpp(radians, CheckSSECpp());
 
    if (MyProc1()) {
        std::cout << "SSE2 is available on this processor." << std::endl;
    }
    else {
        std::cout << "SSE2 is not available on this processor." << std::endl;
    }

    
    std::cout << radians << " radians is " << degrees << std::endl << std::endl;

    std::cout << radians << " radians is " << RadToDegAsm(radians) << "  Dla asm" << std::endl << std::endl;
...
//	int x = 3, y = 4, z = 0;
//
//	z = MyProc1(x, y);		// wywolanie procedury asemblerowej z biblioteki
//
//	std::cout << z;
//
//	return 0;
